...
def test_MPC_algorithm():
    from ..envs.pilco_cartpole import CartPoleSwingUpEnv, pilco_cartpole_reward
    from ..util.control_util import ResettableEnv, get_f_mpc, get_f_batch_mpc
    ...
    plan_env = CartPoleSwingUpEnv()  # ResettableEnv(CartPoleSwingUpEnv())
    f = get_f_batch_mpc(plan_env, use_info_delta=True)
    # f = get_f_mpc(plan_env)
    start_obs = env.reset()
    update_fn = make_update_obs_fn(env, teleport=True, use_tf=False)
    params = dict(start_obs=start_obs, env=plan_env, reward_function=pilco_cartpole_reward, crop_to_domain=True,
                  domain=domain, update_fn=update_fn, base_nsamps=20, planning_horizon=25, n_elites=3,
                  actions_per_plan=6, env_horizon=120)
    mpc = MPC(params)
    mpc.initialize()
