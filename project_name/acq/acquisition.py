...
class JointSetBaxAcqFunction(AlgoAcqFunction):
    ...
    def fast_get_acq_list_batch(x_set, x_data, y_data, base_lmats, base_smats, exe_path_list, kernels, lmats, smats):
        ...
        acq = JointSetBaxAcqFunction.fast_acq_exe_normal(reg_pred_cov, reg_samp_cov_list)
        return acq
    ...
    def get_acq_list_batch(self, x_list):
        ...
        with Timer(f"Compute acquisition function for a batch of {len(x_list)} points", level=logging.DEBUG):
            ...
            for x_set in tqdm(x_list):
                ...
                for i, exe_path in enumerate(self.exe_path_list):
                    ...
                    # NOTE: self.model is multimodel so the following returns a list of mus
                    # and a list of stds
                    samp_mus, samp_stds = self.conditioning_model.get_post_mu_cov(x_set, full_cov=True)
                    mus_list.append(samp_mus)
                    stds_list.append(samp_stds)
        ...
        # Package and store acq_vars
        self.acq_vars = {"mus": mus, "stds": stds, "mus_list": mus_list, "stds_list": stds_list, "acq_list": acq_list}
    ...
    def __call__(self, x_set_list):
        ...
        if self.jit_fast_acq is None:
            ...
            self.jit_fast_acq = jax.vmap(jax.jit(partial(self.fast_get_acq_list_batch,
                                                        ...
                                                        lmats=lmats,
                                                        smats=smats)))
        with Timer(f"Compute fast acquisition function for a batch of {x_set_list.shape[0]} points", level=logging.DEBUG):
            fast_acq_list = self.jit_fast_acq(x_set_list)  # TODO removed fast_acq for now
        not_finites = ~jnp.isfinite(fast_acq_list)
        num_not_finite = jnp.sum(not_finites)
        if num_not_finite > 0:
            logging.warning(f"{num_not_finite} acq function results were not finite.")
            fast_acq_list = fast_acq_list.at[not_finites].set(-np.inf)
        # with Timer(f"Compute slow acquisition function for a batch of {x_set_list.shape[0]} points", level=logging.DEBUG):
        #     slow_acq_list = self.get_acq_list_batch(x_set_list)
        return list(fast_acq_list)


class JointStdBaxAcqFunction(AlgoAcqFunction):
    """
    Class for computing BAX acquisition functions.
    """
    ...
    @staticmethod
    def fast_acq_exe_std(post_covs):
        intermediate = jnp.sum(post_covs, axis=(0, 1))  # TODO what is this 2 dimension idk?
        diags = jnp.diag(intermediate)
        final = jnp.sum(diags, keepdims=True)
        return -final  # To get just variance of posterior, is it +ve or -ve
    ...
    def fast_get_acq_list_batch(x_set, x_data, y_data, base_lmats, base_smats, exe_path_list, kernels, lmats, smats):
        ...
        reg = jnp.eye(samp_cov_list.shape[-1])[None, None, ...] * 1e-5
        reg_pred_cov = pred_cov + reg
        acq = JointStdBaxAcqFunction.fast_acq_exe_std(reg_pred_cov)
        return acq
    ...
    def get_acq_list_batch(self, x_list):
        ...
        with Timer(f"Compute acquisition function for a batch of {len(x_list)} points", level=logging.DEBUG):
            ...
            for x_set in tqdm(x_list):
                ...
                acq_list.append(self.fast_acq_exe_std(jnp.expand_dims(jnp.asarray(stds), axis=0)))

        # Package and store acq_vars
        self.acq_vars = {"mus": mus, "stds": stds, "acq_list": acq_list}
    ...
    def __call__(self, x_set_list):
        ...
        if self.jit_fast_acq is None:
            ...
            self.jit_fast_acq = jax.vmap(jax.jit(partial(self.fast_get_acq_list_batch,
                                                        ...
                                                        lmats=lmats,
                                                        smats=smats)))
        with Timer(f"Compute fast acquisition function for a batch of {x_set_list.shape[0]} points", level=logging.DEBUG):  # level=logging.INFO  for printing at every step
            fast_acq_list = self.jit_fast_acq(x_set_list)  # TODO removed fast_acq for now
        not_finites = ~jnp.isfinite(fast_acq_list)
        num_not_finite = jnp.sum(not_finites)
        if num_not_finite > 0:
        ...
        with Timer(f"Compute slow acquisition function for a batch of {x_set_list.shape[0]} points", level=logging.DEBUG):
            slow_acq_list = self.get_acq_list_batch(x_set_list)
        # return list(slow_acq_list), list(fast_acq_list)  # Basically take similar time, well 7 second difference
        return list(slow_acq_list)
